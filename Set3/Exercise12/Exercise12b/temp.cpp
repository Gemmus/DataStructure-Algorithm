...
void fft_single(int n, cx x[]) {
    ...
    // Run the recursive calls on a single thread
    thread t1(fft_single, n2, xe);
    t1.join();
    thread t2(fft_single, n2, xo);
    t2.join();
...
int main() {
    ...
    //output_file << ;
    cout << setw(7) << right << " N " << setw(10) << right << "Unopt" << setw(14) << right << "Single" << setw(14) << right << "Parallel"<< endl;

    const int min_N = 2048;
    const int max_N = 262144;
    double    mean_unopt,    mean_unopt_br,    mean_single,    mean_single_br,    mean_paral,    mean_paral_br;
    double   stdev_unopt,   stdev_unopt_br,   stdev_single,   stdev_single_br,   stdev_paral,   stdev_paral_br;

    for (int N = min_N; N <= max_N; N *= 2) {
        auto* unopt = new FFTTestbed(fft_parallel, N);
        unopt->measure(mean_unopt, stdev_unopt);
        auto* single = new FFTTestbed(fft_single, N);
        single->measure(mean_single, stdev_single);
        auto* paral = new FFTTestbed(fft_parallel, N);
        paral->measure(mean_paral, stdev_paral);
        cout << setw(7) << right <<  N << ": " << setw(2) << right << (int)mean_unopt << "(+/- " << (int)stdev_unopt*2 << ")ms" << setw(4) << right << (int)mean_single << "(+/- " << (int)stdev_single*2 << ")ms" << setw(4) << right << (int)mean_paral << "(+/- " << (int)stdev_paral*2 << ")ms"  << endl;
        delete unopt; delete single; delete paral;
    }
